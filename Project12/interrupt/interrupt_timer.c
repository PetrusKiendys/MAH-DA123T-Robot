...
#define	PERFORM_AD_READ		CUSTOMSETTINGS_PERFORM_AD_READ
#define DEBUG				CUSTOMSETTINGS_DEBUG
#define	MULTIPLIER			CUSTOMSETTINGS_TMR1_ISR_COUNTER_RESET_MULTIPLIER
...
{
	static tU16 counter_timer1 = 0;

		if (DEBUG) {
			counter_timer1++;
			if (counter_timer1 == CUSTOMSETTINGS_SAMPLE_FREQ*MULTIPLIER) {
				printf("\ncounter_timer1: %d", counter_timer1);
				counter_timer1 = 0;
			}
		}
	...
	if (PERFORM_AD_READ) {
		...
		while ((ADDR & 0x80000000) == 0)
			;  //vänta till AD-omvandlaren är klar
